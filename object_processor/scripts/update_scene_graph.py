...
class update_scene_graph:
    def __init__(self):
        ...
        self.input_scene_graph = rospy.get_param('~input_scene_graph')
        self.output_scene_graph_name = rospy.get_param('~output_scene_graph_name')
        self.min_num_detections = rospy.get_param('~min_num_detections')
        self.time_buffer_secs = rospy.get_param('~time_buffer_secs')
        self.reset_time_secs = rospy.get_param('~reset_time_secs')
        self.segmentation_topic = rospy.get_param('~segmentation_topic')
        self.image = None
        self.object_maps = {}
        self.color_maps = {}
        self.static_objects = []
        ...
        self.connections = [] 
        self.read_pkl(self.input_scene_graph)
        ...
        self.sub = rospy.Subscriber(self.segmentation_topic, Image, self.image_callback)
    ...
    def visualize_scene_graphs(self,objects):
        if len(objects)>0:
            ...
            if len(unknown_objects) > 0:
                self.G.add_edge(self.room,"unknown")
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            print("Length of connections ", len(self.connections))
            try:
                trans = tfBuffer.lookup_transform(self.world_frame, self.camera_frame, rospy.Time.now(),rospy.Duration(1.0))
            except (tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException):
                rate.sleep()
                continue
            for i in range(len(self.connections)):
                if rospy.Time.now().secs - self.connections[i].time_stamp < self.reset_time_secs:
                    continue
                print("HELLO")
                object_in_world = PoseStamped()
                object_in_world.header.frame_id = self.world_frame
                ...
                object_in_camera = tfBuffer.transform(object_in_world, self.camera_frame, timeout=rospy.Duration(10.0)) #point on camera_color_optical_frame
                ...
                if object_in_camera.pose.position.z > 0:
                    ...
                    if self.connections[i].back_in_view:
                        if (rospy.get_rostime().secs - self.connections[i].back_in_view_time)<self.time_buffer_secs:    
                            if ([int(x) for x in self.color_maps[str(self.connections[i].pair[0])]]==self.image).all(axis=2).sum() != 0:
                                self.connections[i].no_of_det = self.connections[i].no_of_det + 1
                            print("Number of detections ", self.connections[i].no_of_det)
                        else:
                            if self.connections[i].no_of_det < self.min_num_detections:
    ...
    def handler(self,signum,frame):
        if len(self.connections)>0:
            with open(self.scene_graph_path + self.output_scene_graph_name, 'wb') as outp:
                    print("DUMPING")
